...
class CUDADocScraper:
    ...
    async def scrape_warp_apis_fallback(self) -> List[CUDAAPIScrapedData]:
        """
        直接使用 fallback 数据采集 Warp Shuffle/Vote/Reduce/Match APIs（跳过浏览器）

        注意：不包含 Thread Sync 和 Memory Fence，它们需要单独采集
        ...
        logger.info(f"Collected {len(apis)} warp shuffle/vote/reduce/match APIs from fallback")
        return apis
    ...
    def _get_fallback_api_data(self, api_name: str) -> Optional[CUDAAPIScrapedData]:
        ...
        fallback_data = {
            ...
            # Legacy warp vote (deprecated)
            "__all": CUDAAPIScrapedData(
                api_name="__all",
                full_signature="int __all(int predicate)",
                description="[DEPRECATED] Warp-level ALL reduction. Returns 1 if predicate is true for all threads in the warp.",
                category="warp-vote",
                subcategory="vote",
            ...
            "__any": CUDAAPIScrapedData(
                api_name="__any",
                full_signature="int __any(int predicate)",
                description="[DEPRECATED] Warp-level ANY reduction. Returns 1 if predicate is true for any thread in the warp.",
                category="warp-vote",
                subcategory="vote",
                parameters=["int predicate"],
                return_type="int",
            ...
            "__match_any_sync": CUDAAPIScrapedData(
                ...
                description="Returns mask of threads in warp where var matches any other thread's value.",
                category="warp-match",
                subcategory="match",
                parameters=["unsigned mask", "T var"],
                return_type="unsigned",
                documentation_url=f"{WARP_SHUFFLE_BASE_URL}#warp-match-functions",
            ),
    ...
    def _get_fallback_memory_api_data(self, api_name: str) -> Optional[CUDAAPIScrapedData]:
        ...
        fallback_data = {
            ...
            "cudaMallocMipmappedArray": CUDAAPIScrapedData(
                api_name="cudaMallocMipmappedArray",
                full_signature="cudaError_t cudaMallocMipmappedArray(cudaMipmappedArray_t* mipmapArray, const cudaChannelFormatDesc* desc, cudaExtent extent, unsigned int numLevels)",
                description="Allocates a mipmapped array on the GPU device.",
                category="memory-management",
                subcategory="allocation",
                parameters=["cudaMipmappedArray_t* mipmapArray", "const cudaChannelFormatDesc* desc", "cudaExtent extent", "unsigned int numLevels"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__MEMORY.html",
            ),
            "cudaFreeMipmappedArray": CUDAAPIScrapedData(
                api_name="cudaFreeMipmappedArray",
                full_signature="cudaError_t cudaFreeMipmappedArray(cudaMipmappedArray_t mipmapArray)",
                description="Frees a mipmapped array on the GPU device.",
                category="memory-management",
                subcategory="deallocation",
                parameters=["cudaMipmappedArray_t mipmapArray"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__MEMORY.html",
            ),
            "cudaHostAlloc": CUDAAPIScrapedData(
                api_name="cudaHostAlloc",
                full_signature="cudaError_t cudaHostAlloc(void** pHost, size_t size, unsigned int flags)",
                description="Allocates page-locked host memory with specified flags.",
                category="memory-management",
                subcategory="host-memory",
                parameters=["void** pHost", "size_t size", "unsigned int flags"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__MEMORY.html",
            ),
            "cudaHostRegister": CUDAAPIScrapedData(
                api_name="cudaHostRegister",
                full_signature="cudaError_t cudaHostRegister(void* ptr, size_t size, unsigned int flags)",
                description="Registers an existing host memory range for use with CUDA.",
                category="memory-management",
                subcategory="host-memory",
                parameters=["void* ptr", "size_t size", "unsigned int flags"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__MEMORY.html",
            ),
            "cudaHostUnregister": CUDAAPIScrapedData(
                api_name="cudaHostUnregister",
                full_signature="cudaError_t cudaHostUnregister(void* ptr)",
                description="Unregisters a host memory range that was registered with CUDA.",
                category="memory-management",
                subcategory="host-memory",
                parameters=["void* ptr"],
                return_type="cudaError_t",
            ...
            "cudaMemGetInfo": CUDAAPIScrapedData(
                api_name="cudaMemGetInfo",
                full_signature="cudaError_t cudaMemGetInfo(size_t* free, size_t* total)",
                description="Returns free and total device memory information.",
                category="memory-management",
                subcategory="query",
                parameters=["size_t* free", "size_t* total"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__MEMORY.html",
            ),
            "cudaMemAdvise": CUDAAPIScrapedData(
                api_name="cudaMemAdvise",
                full_signature="cudaError_t cudaMemAdvise(const void* devPtr, size_t count, cudaMemoryAdvise advice, int device)",
                description="Advises the CUDA driver about the usage pattern for a memory region.",
                category="memory-management",
                subcategory="management",
                parameters=["const void* devPtr", "size_t count", "cudaMemoryAdvise advice", "int device"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__MEMORY.html",
            ),
            "cudaMemPrefetchAsync": CUDAAPIScrapedData(
                api_name="cudaMemPrefetchAsync",
                full_signature="cudaError_t cudaMemPrefetchAsync(const void* devPtr, size_t count, int dstDevice, cudaStream_t stream)",
                description="Prefetches memory to the specified device.",
                category="memory-management",
                subcategory="management",
                parameters=["const void* devPtr", "size_t count", "int dstDevice", "cudaStream_t stream"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__MEMORY.html",
            ),
            ...
            "cudaStreamCreateWithPriority": CUDAAPIScrapedData(
                api_name="cudaStreamCreateWithPriority",
                full_signature="cudaError_t cudaStreamCreateWithPriority(cudaStream_t* stream, unsigned int flags, int priority)",
                description="Creates a stream with specified priority.",
                category="stream-management",
                subcategory="create",
                parameters=["cudaStream_t* stream", "unsigned int flags", "int priority"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__STREAM.html",
            ),
            "cudaStreamGetDevice": CUDAAPIScrapedData(
                api_name="cudaStreamGetDevice",
                full_signature="cudaError_t cudaStreamGetDevice(cudaStream_t stream, int* device)",
                description="Returns the device associated with a stream.",
                category="stream-management",
                subcategory="query",
                parameters=["cudaStream_t stream", "int* device"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__STREAM.html",
            ),
            "cudaStreamGetFlags": CUDAAPIScrapedData(
                api_name="cudaStreamGetFlags",
                full_signature="cudaError_t cudaStreamGetFlags(cudaStream_t stream, unsigned int* flags)",
                description="Returns the flags associated with a stream.",
                category="stream-management",
                subcategory="query",
                parameters=["cudaStream_t stream", "unsigned int* flags"],
                return_type="cudaError_t",
            ...
            "cudaStreamGetId": CUDAAPIScrapedData(
                api_name="cudaStreamGetId",
                full_signature="cudaError_t cudaStreamGetId(cudaStream_t stream, cuuint64_t* streamId)",
                description="Returns the unique ID for a stream.",
                category="stream-management",
                subcategory="query",
                parameters=["cudaStream_t stream", "cuuint64_t* streamId"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__STREAM.html",
            ),
            "cudaStreamGetPriority": CUDAAPIScrapedData(
                api_name="cudaStreamGetPriority",
                full_signature="cudaError_t cudaStreamGetPriority(cudaStream_t stream, int* priority)",
                description="Returns the priority of a stream.",
                category="stream-management",
                subcategory="query",
                parameters=["cudaStream_t stream", "int* priority"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__STREAM.html",
            ),
            "cudaStreamSetAttribute": CUDAAPIScrapedData(
                api_name="cudaStreamSetAttribute",
                full_signature="cudaError_t cudaStreamSetAttribute(cudaStream_t stream, cudaStreamAttrID attr, void* value)",
                description="Sets an attribute on a stream.",
                category="stream-management",
                subcategory="attribute",
                parameters=["cudaStream_t stream", "cudaStreamAttrID attr", "void* value"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__STREAM.html",
            ),
            "cudaStreamGetAttribute": CUDAAPIScrapedData(
                api_name="cudaStreamGetAttribute",
                full_signature="cudaError_t cudaStreamGetAttribute(cudaStream_t stream, cudaStreamAttrID attr, void* value)",
                description="Gets an attribute on a stream.",
                category="stream-management",
                subcategory="attribute",
                parameters=["cudaStream_t stream", "cudaStreamAttrID attr", "void* value"],
                return_type="cudaError_t",
            ...
            "cudaStreamCopyAttributes": CUDAAPIScrapedData(
                api_name="cudaStreamCopyAttributes",
                full_signature="cudaError_t cudaStreamCopyAttributes(cudaStream_t dst, cudaStream_t src)",
                description="Copies attributes from one stream to another.",
                category="stream-management",
                subcategory="attribute",
                parameters=["cudaStream_t dst", "cudaStream_t src"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__STREAM.html",
            ),
            "cudaStreamUpdateCaptureDependencies": CUDAAPIScrapedData(
                api_name="cudaStreamUpdateCaptureDependencies",
                full_signature="cudaError_t cudaStreamUpdateCaptureDependencies(cudaStream_t stream, cudaGraphNode_t* dependencies, size_t numDependencies, unsigned int flags)",
                description="Updates capture dependencies for a stream.",
                category="stream-management",
                subcategory="capture",
                parameters=["cudaStream_t stream", "cudaGraphNode_t* dependencies", "size_t numDependencies", "unsigned int flags"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__STREAM.html",
            ),
            "cudaThreadExchangeStreamCaptureMode": CUDAAPIScrapedData(
                api_name="cudaThreadExchangeStreamCaptureMode",
                full_signature="cudaError_t cudaThreadExchangeStreamCaptureMode(cudaStreamCaptureMode* mode)",
                description="Exchanges the stream capture mode for the calling thread.",
                category="stream-management",
                subcategory="capture",
                parameters=["cudaStreamCaptureMode* mode"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__STREAM.html",
            ),
            "cudaCtxResetPersistingL2Cache": CUDAAPIScrapedData(
                api_name="cudaCtxResetPersistingL2Cache",
                full_signature="cudaError_t cudaCtxResetPersistingL2Cache()",
                description="Resets the persisting L2 cache.",
                category="stream-management",
                subcategory="cache",
                parameters=[],
                return_type="cudaError_t",
            ...
            "cudaEventRecordWithFlags": CUDAAPIScrapedData(
                api_name="cudaEventRecordWithFlags",
                full_signature="cudaError_t cudaEventRecordWithFlags(cudaEvent_t event, cudaStream_t stream, unsigned int flags)",
                description="Records an event in a stream with specified flags.",
                category="event-management",
                subcategory="record",
                parameters=["cudaEvent_t event", "cudaStream_t stream", "unsigned int flags"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__EVENT.html",
            ),
            # Device APIs
    ...
    def _get_fallback_device_api_data(self, api_name: str) -> Optional[CUDAAPIScrapedData]:
        ...
        fallback_data = {
            ...
            # Additional Device APIs
            "cudaGetDeviceFlags": CUDAAPIScrapedData(
                api_name="cudaGetDeviceFlags",
                full_signature="cudaError_t cudaGetDeviceFlags(unsigned int* flags)",
                description="Returns the current device flags.",
                category="device-management",
                subcategory="query",
            ...
            "cudaInitDevice": CUDAAPIScrapedData(
                api_name="cudaInitDevice",
                full_signature="cudaError_t cudaInitDevice(int device)",
                description="Initializes a device for the calling host thread.",
                category="device-management",
            ...
            "cudaSetDeviceFlags": CUDAAPIScrapedData(
                api_name="cudaSetDeviceFlags",
                full_signature="cudaError_t cudaSetDeviceFlags(unsigned int flags)",
                description="Sets the current device flags.",
                category="device-management",
            ...
            "cudaSetValidDevices": CUDAAPIScrapedData(
                api_name="cudaSetValidDevices",
                full_signature="cudaError_t cudaSetValidDevices(int* deviceArray, int count)",
                description="Sets valid devices for the calling host thread.",
                category="device-management",
                subcategory="selection",
                parameters=["int* deviceArray", "int count"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__DEVICE.html",
            ),
            "cudaDeviceGetAttribute": CUDAAPIScrapedData(
                api_name="cudaDeviceGetAttribute",
                full_signature="cudaError_t cudaDeviceGetAttribute(int* value, cudaDeviceAttr attr, int device)",
                description="Returns information about a device attribute.",
                category="device-management",
                subcategory="attribute",
                parameters=["int* value", "cudaDeviceAttr attr", "int device"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__DEVICE.html",
            ),
            "cudaDeviceGetByPCIBusId": CUDAAPIScrapedData(
                api_name="cudaDeviceGetByPCIBusId",
                full_signature="cudaError_t cudaDeviceGetByPCIBusId(int* device, const char* pciBusId)",
                description="Returns the device associated with a PCI bus ID.",
                category="device-management",
                subcategory="pci",
                parameters=["int* device", "const char* pciBusId"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__DEVICE.html",
            ),
            "cudaDeviceGetPCIBusId": CUDAAPIScrapedData(
                api_name="cudaDeviceGetPCIBusId",
                full_signature="cudaError_t cudaDeviceGetPCIBusId(char* pciBusId, int len, int device)",
                description="Returns the PCI bus ID of a device.",
                category="device-management",
                subcategory="pci",
                parameters=["char* pciBusId", "int len", "int device"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__DEVICE.html",
            ),
            "cudaDeviceGetStreamPriorityRange": CUDAAPIScrapedData(
                api_name="cudaDeviceGetStreamPriorityRange",
                full_signature="cudaError_t cudaDeviceGetStreamPriorityRange(int* leastPriority, int* greatestPriority)",
                description="Returns the range of stream priorities.",
                category="device-management",
                subcategory="stream",
                parameters=["int* leastPriority", "int* greatestPriority"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__DEVICE.html",
            ),
            "cudaDeviceGetDefaultMemPool": CUDAAPIScrapedData(
                api_name="cudaDeviceGetDefaultMemPool",
                full_signature="cudaError_t cudaDeviceGetDefaultMemPool(cudaMemPool_t* memPool, int device)",
                description="Returns the default memory pool for a device.",
                category="device-management",
                subcategory="memory-pool",
                parameters=["cudaMemPool_t* memPool", "int device"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__DEVICE.html",
            ),
            "cudaDeviceGetMemPool": CUDAAPIScrapedData(
                api_name="cudaDeviceGetMemPool",
                full_signature="cudaError_t cudaDeviceGetMemPool(cudaMemPool_t* memPool, int device)",
                description="Returns the current memory pool for a device.",
                category="device-management",
                subcategory="memory-pool",
                parameters=["cudaMemPool_t* memPool", "int device"],
                return_type="cudaError_t",
            ...
            "cudaDeviceSetMemPool": CUDAAPIScrapedData(
                api_name="cudaDeviceSetMemPool",
                full_signature="cudaError_t cudaDeviceSetMemPool(int device, cudaMemPool_t memPool)",
                description="Sets the memory pool for a device.",
                category="device-management",
                subcategory="memory-pool",
                parameters=["int device", "cudaMemPool_t memPool"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__DEVICE.html",
            ),
            "cudaDeviceFlushGPUDirectRDMAWrites": CUDAAPIScrapedData(
                api_name="cudaDeviceFlushGPUDirectRDMAWrites",
                full_signature="cudaError_t cudaDeviceFlushGPUDirectRDMAWrites(cudaGPUDirectRDMAWritesOrdering ordering, int device)",
                description="Flushes GPU direct RDMA writes.",
                category="device-management",
                subcategory="rdma",
                parameters=["cudaGPUDirectRDMAWritesOrdering ordering", "int device"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__DEVICE.html",
            ),
            # IPC APIs
            "cudaIpcCloseMemHandle": CUDAAPIScrapedData(
                api_name="cudaIpcCloseMemHandle",
                full_signature="cudaError_t cudaIpcCloseMemHandle(void* devPtr)",
                description="Closes a memory handle for IPC.",
                category="device-management",
                subcategory="ipc",
                parameters=["void* devPtr"],
                return_type="cudaError_t",
            ...
            "cudaIpcGetEventHandle": CUDAAPIScrapedData(
                api_name="cudaIpcGetEventHandle",
                full_signature="cudaError_t cudaIpcGetEventHandle(cudaIpcEventHandle_t* handle, cudaEvent_t event)",
                description="Gets an event handle for IPC.",
                category="device-management",
                subcategory="ipc",
                parameters=["cudaIpcEventHandle_t* handle", "cudaEvent_t event"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__DEVICE.html",
            ),
            "cudaIpcGetMemHandle": CUDAAPIScrapedData(
                api_name="cudaIpcGetMemHandle",
                full_signature="cudaError_t cudaIpcGetMemHandle(cudaIpcMemHandle_t* handle, void* devPtr)",
                description="Gets a memory handle for IPC.",
                category="device-management",
                subcategory="ipc",
                parameters=["cudaIpcMemHandle_t* handle", "void* devPtr"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__DEVICE.html",
            ),
            "cudaIpcOpenEventHandle": CUDAAPIScrapedData(
                api_name="cudaIpcOpenEventHandle",
                full_signature="cudaError_t cudaIpcOpenEventHandle(cudaEvent_t* event, cudaIpcEventHandle_t handle)",
                description="Opens an event handle for IPC.",
                category="device-management",
                subcategory="ipc",
                parameters=["cudaEvent_t* event", "cudaIpcEventHandle_t handle"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__DEVICE.html",
            ),
            "cudaIpcOpenMemHandle": CUDAAPIScrapedData(
                api_name="cudaIpcOpenMemHandle",
                full_signature="cudaError_t cudaIpcOpenMemHandle(void** devPtr, cudaIpcMemHandle_t handle, unsigned int flags)",
                description="Opens a memory handle for IPC.",
                category="device-management",
                subcategory="ipc",
                parameters=["void** devPtr", "cudaIpcMemHandle_t handle", "unsigned int flags"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__DEVICE.html",
            ),
            "cudaDeviceGetP2PAttribute": CUDAAPIScrapedData(
                api_name="cudaDeviceGetP2PAttribute",
                full_signature="cudaError_t cudaDeviceGetP2PAttribute(int* value, cudaDeviceP2PAttr attr, int srcDevice, int dstDevice)",
                description="Returns information about P2P access capability between devices.",
                category="device-management",
                subcategory="p2p",
                parameters=["int* value", "cudaDeviceP2PAttr attr", "int srcDevice", "int dstDevice"],
                return_type="cudaError_t",
                documentation_url="https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__DEVICE.html",
            ),
            # Warp Match APIs
